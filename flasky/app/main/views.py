...
def webchat(nickname):
    ...
    if user is None or user == current_user or not current_user.is_friend(user) :
        ...
        return redirect(url_for('main.index'))
    toshows = Message.query.filter(and_(or_(and_(Message.sender_id==user.id, Message.receiver_id==current_user.id), and_(Message.sender_id==current_user.id, Message.receiver_id==user.id)), Message.room_id==None)).order_by(Message.time_stamp).limit(500).all()
    if len(toshows):
...
def roomchat(id):
    ...
    db.session.commit()
    toshows = room.messages.filter_by(state="sent").order_by(Message.time_stamp).limit(1000).all()
